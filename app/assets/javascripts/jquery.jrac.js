        ...
        if (!jrac_loaded) {
          ...
          .slider({
            ...
            slide: function(event, ui) {
              ...
              if ((height < settings.crop_height) || (ui.value < settings.crop_width)) return false;
              $image.height(height);
              ...
              var img_pos = $image.position();

              if ($image.width() + img_pos.left < settings.crop_width) {
                $image.css({'left': settings.crop_width - $image.width()});
                $viewport.observator.notify('crop_x', $viewport.observator.crop_position_x());
              }

              if ($image.height() + img_pos.top < settings.crop_height) {
                $image.css({'top': settings.crop_height - $image.height()});
                $viewport.observator.notify('crop_y', $viewport.observator.crop_position_y());
              }

            }
          });
          $container.prepend($zoom_widget);
          ...
          $image.draggable({
            ...
            drag: function(event, ui) {
              ...
              if ((ui.position.left + $image.width()) < settings.crop_width) ui.position.left = ( settings.crop_width - $image.width() );


              if (ui.position.top > 0) ui.position.top = 0;

              if ((ui.position.top + $image.height()) < settings.crop_height) {
                ui.position.top = settings.crop_height - $image.height();
              }
          ...
          .css({
            'width': $image.width(), //settings.crop_width,
            'height': $image.height(), //settings.crop_height,
